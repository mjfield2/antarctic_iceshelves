...
dir_path = Path('G:/antarctic_iceshelves/iceshelves/getz')

ds = xr.load_dataset(dir_path/'getz.nc')
grav = pd.read_csv(dir_path/'getz_grav.csv')
...
print(f'running {n_invs} inversions of Getz')
...
for i in tqdm(range(n_invs)):
    ...
    path = Path(dir_path/f'results/result_{i}.npy')
