...
def mm1_solve(sim_xy, nearest, nearest_secondary, vario, global_mean, global_mean_secondary, global_var, global_var_secondary, corrcoef, rcond=None, precompute=False):
    """
    Solve simple kriging system given neighboring points and secondary variable
    ...
    xy_val = np.vstack((nearest[:,:2], nearest_secondary[:2].reshape(1, 2)))
    local_mean = np.mean(nearest[:,2])
    n = nearest.shape[0]

    # print(nearest.shape, nearest_secondary.shape)

    # xy_val = nearest[:,:2]
    # xy_second = nearest_secondary[:,:2]
    # print(xy_val.shape, xy_second.shape)
    # xy_val2 = np.append(xy_val, [xy_second], axis = 0)
    # print(xy_val2.shape)

    # data_vals = np.concatenate((nearest[:,2], nearest_secondary[2]))
    data_vals = np.append(nearest[:,2], nearest_secondary[2])

    # covariance between data
    Sigma = np.zeros((n+1, n+1))
    Sigma[0:n+1,0:n+1] = make_sigma(xy_val, rotation_matrix, vario)

    # Set up Right Hand Side (covariance between data and unknown)
    rho = np.zeros((n+1))
    rho[0:n+1] = make_rho(xy_val, sim_xy, rotation_matrix, vario)
    rho *= corrcoef

    # update covariance matrix with secondary info
    Sigma[n,0:n+1] = Sigma[n,0:n+1] * corrcoef
    Sigma[0:n+1,n] = Sigma[0:n+1,n] * corrcoef
    Sigma[n,n] = 1

    # solve for kriging weights
    k_weights, res, rank, s = np.linalg.lstsq(Sigma, rho, rcond=rcond) 
    part1 = global_mean + np.sum(k_weights[0:n]*(data_vals[0:n]-global_mean)/np.sqrt(global_var))
    part2 = k_weights[n] * (nearest_secondary[2] - global_mean_secondary)/np.sqrt(global_var_secondary)
    var = global_var - np.sum(k_weights*rho)
