...
def krige(xx, yy, grid, variogram, radius=100e3, num_points=20, ktype='ok', sim_mask=None, quiet=False, stencil=None):
    ...
    out_grid, nst_trans, cond_msk, inds, vario, global_mean, stencil, bounds = _preprocess(xx, yy, grid, variogram, sim_mask, radius, stencil, bounds=None)
...
def cosim_mm1(xx, yy, grid, secondary, variogram, corrcoef, radius=100e3, num_points=20, ktype='ok', sim_mask=None, quiet=False, stencil=None, rcond=None, bounds=None, seed=None):
    """
    Sequential Gaussian Simulation with ordinary or simple kriging using nearest neighbors found in an octant search.
    ...
    # mean of secondary
    global_mean_secondary = np.nanmean(secondary)
    global_var = np.nanvar(out_grid)
    global_var_secondary = np.nanvar(secondary)
    ...
    for k in tqdm(range(inds.shape[0]), disable=quiet):
        ...
        if cond_msk[i, j] == False:
            ...
            while nearest.shape[0] == 0:
                nearest = neighbors(i, j, ii, jj, xx, yy, out_grid, cond_msk, rad, num_points, stencil=stenc)
                nearest_secondary = neighbors(i, j, ii, jj, xx, yy, secondary, cond_msk, rad, num_points, stencil=stenc)[0,:]
                if nearest.shape[0] > 0:
                    break
                else:
                    rad += 100e3
            ...
            est, var = mm1_solve((xx[i,j], yy[i,j]), nearest, nearest_secondary, local_vario, global_mean, global_mean_secondary, 
                                 global_var, global_var_secondary, corrcoef, rcond)
