...
def lowpass_filter(x, y, grid, cutoff=10e3, pad_size=3):
    """
    Gaussian low-pass filter 2D grid

    Args:
        x : x-coordinates of grid,
        y : y-coordinates of grid,
        grid : 2D numpy array to filter,
        cutoff : pass frequencies below this
    Outputs:
        low, high : low frequencies and high frequencies
    """
    ...
    pad_width = {
        "x": grid_da.x.size // pad_size,
        "y": grid_da.y.size // pad_size,
    }
    ...
    return grid_low_freqs, grid_high_freqs

def lowpass_filter_invpad(ds, grid, cutoff=10e3, pad=0):
    """
    Lowpass filter grid within inversion domain with optional padding.
    Positive padding pushes inversion domain inwards.

    Args:
        ds : preprocessed BedMachine xarray.Dataset
        grid : 2D numpy array to filter,
        cutoff : pass frequencies below this
        pad : distance inside of inversion domain to keep original grid
    Outputs:
        grid filtered inside of inversion domain with optional padding
    """

    xx, yy = np.meshgrid(ds.x, ds.y)
    
    grid_low_freqs, grid_high_freqs = lowpass_filter(ds.x.values, ds.y.values, grid, cutoff)

    inv_msk_pad = vd.distance_mask(
        data_coordinates = (xx[ds.inv_msk==True], yy[ds.inv_msk==True]),
        maxdist=pad,
        coordinates = (xx, yy)
    )
    
    grid_mod = np.where(inv_msk_pad==True, grid_low_freqs, grid)
...
def ant2geo(x, y):
    """
    Converts Antarctic Polar Stereographic coordinates to geodetic
    ...
    Outputs:
        lat, lon : latitude and longitude arrays
    """
    transformer = Transformer.from_crs(3031, 4326)
    lat, lon = transformer.transform(x, y)
    return lat, lon

def get_topo_cm():
    """
    Returns custom diverging colormap for representing water and terrain
    """
