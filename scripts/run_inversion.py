...
print(f'Running inversions for {args.name} form index {args.low} to {args.high}')
print(f'lambda: {lopt}')
...
for i in range(args.low, args.high):
    ...
    if args.name in ['larsen', 'maudeast', 'maudwest']:
    ...
    bed_cache, rmse_cache = gauss_newton_regularized(ds, prior, target, pred_coords, density_dict, Cm, Wd, max_iter=30, alpha='search', perturb_scale=1, lamb=lopt, stop=0.01, quiet=True)
